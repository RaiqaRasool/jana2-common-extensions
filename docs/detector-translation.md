...
1. Load one detector mapping JSON file into a `TranslationTable`.
...
- Detector mapping JSON files are installed under
  `config/<namespace>/evio_parser/detector_mappings/`.
- Mapping changes are expected only between runs.

## Failure Behavior

Loading throws `std::runtime_error` when the file cannot be read, the JSON or
required schema is invalid, an integer is out of range, or a DAQ address is
duplicated.
...
The `translation_table_tests` CTest loads the demo HMS mapping, verifies its
known lookup, duplicate insertion rejection, and an unknown-address lookup.
